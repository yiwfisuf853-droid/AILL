 ...
 *   post:
 *     tags: [上传]
 *     summary: "单文件上传(multipart/form-data, 返回4尺寸WebP)"
 *     security: [{ BearerAuth: [] }]
 *     requestBody:
 *       required: true
 *       content:
...
router.post('/', upload.single('file'), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // 删除临时文件
    try { fs.unlinkSync(tmpPath); } catch (e) { console.warn('[Upload] Failed to delete temp file:', e.message); }
    ...
    // 确保临时文件被清理
    try { fs.unlinkSync(tmpPath); } catch (e) { console.warn('[Upload] Failed to delete temp file on error:', e.message); }
    throw err;
  }
}));
