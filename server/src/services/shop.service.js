...
// 订单状态：兼容 schema.sql 中 orders.status 的 varchar 定义
const ORDER_STATUS = {
  PENDING: 'pending',
  PAID: 'paid',
  COMPLETED: 'completed',
...
function normalizeOrderStatus(status) {
  ...
  if (status === 1 || status === '1') return ORDER_STATUS.PENDING;
  if (status === 2 || status === '2') return ORDER_STATUS.PAID;
  if (status === 3 || status === '3') return ORDER_STATUS.COMPLETED;
  if (status === 4 || status === '4') return ORDER_STATUS.CANCELLED;
  return status;
}
...
export async function createOrder(userId, data) {
  ...
  try {
    ...
    const orderRes = await client.query(
      ...
      [orderId, userId, totalAmount, totalPoints, ORDER_STATUS.PENDING, null, null, data.remark || '', now, now]
    );
...
export async function payOrder(userId, orderId, data) {
  ...
  if (!isPendingOrder(order.status)) throw new AppError('订单状态不允许支付', 400);
...
export async function cancelOrder(userId, orderId) {
  ...
  if (!order) throw new NotFoundError('订单不存在');
  if (!isPendingOrder(order.status)) throw new AppError('只能取消待支付订单', 400);
