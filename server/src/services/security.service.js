...
export async function checkLoginRestriction(identifier, ip) {
  ...
  const result = await repo.rawQuery(
    `SELECT COUNT(*) as cnt FROM login_attempts WHERE identifier = $1 AND is_success = 0 AND created_at > NOW() - INTERVAL '30 minutes'`,
    [identifier]
  );
...
async function checkAndAutoBlock(data) {
  // 最近1小时内失败5次自动封IP
  const result = await repo.rawQuery(
    `SELECT COUNT(*) as cnt FROM login_attempts WHERE identifier = $1 AND is_success = 0 AND created_at > NOW() - INTERVAL '1 hour'`,
    [data.identifier]
  );
  const failCount = Number(result.rows[0].cnt);
...
export async function getFileMetadata(options = {}) {
  ...
  return await repo.findAll('file_metadata', { where, page, limit, orderBy: 'created_at DESC' });
}
