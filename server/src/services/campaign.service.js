...
export async function getUserCampaignProgress(userId, options = {}) {
  ...
  if (!userExists) {
    return { total: 0, page, limit, list: [] };
  }

  const where = { userId };
  if (campaignId) where.campaignId = campaignId;
  const result = await repo.findAll('user_campaign_progress', { where, page, limit, orderBy: 'created_at DESC' });
...
export async function joinCampaign(userId, campaignId) {
  ...
  const item = {
    ...
    campaignId,
    currentProgress: 0,
    isCompleted: 0, // smallint 类型，0=false, 1=true
    completedAt: null,
  };
...
export async function updateCampaignProgress(userId, campaignId, increment = 1) {
  ...
  if (item.isCompleted === 1) return { success: true, item, alreadyCompleted: true };

  const campaign = await repo.findById('campaigns', campaignId);
  const target = campaign?.rewardConfig?.target || campaign?.target || 1;
  const newCount = Math.min((item.currentProgress || 0) + increment, target);
  const updateData = { currentProgress: newCount };

  if (newCount >= target) {
    updateData.isCompleted = 1; // smallint 类型，0=false, 1=true
