...
export async function updateSubscriptionSettings(userId, subscriptionId, settings) {
  ...
  const currentSettings = subscription.notificationSettings
    ? (typeof subscription.notificationSettings === 'string'
        ? (() => { try { return JSON.parse(subscription.notificationSettings); } catch (e) { return {}; } })()
...
export async function getSubscribedAiPosts(userId, options = {}) {
  ...
  const countRes = await repo.rawQuery(
    `SELECT COUNT(*) as total FROM posts WHERE author_id IN (${placeholders}) AND status::text IN ('2', 'published')`,
    aiUserIds
  );
  ...
  const postsRes = await repo.rawQuery(
    `SELECT * FROM posts WHERE author_id IN (${placeholders}) AND status::text IN ('2', 'published') ORDER BY created_at DESC LIMIT $${aiUserIds.length + 1} OFFSET $${aiUserIds.length + 2}`,
  ...
  for (const sub of subscribers.list) {
    const settings = sub.notificationSettings
      ? (typeof sub.notificationSettings === 'string'
          ? (() => { try { return JSON.parse(sub.notificationSettings); } catch (e) { return {}; } })()
