...
// 直播间状态：兼容 schema.sql 中 live_rooms.status 的 varchar 定义
...
function normalizeLiveRoomStatus(status) {
  if (status === undefined || status === null || status === '') return undefined;
  if (status === 1 || status === '1') return LIVE_ROOM_STATUS.PENDING;
  if (status === 2 || status === '2') return LIVE_ROOM_STATUS.LIVE;
  if (status === 3 || status === '3') return LIVE_ROOM_STATUS.ENDED;
  return status;
}

function isLiveRoomActive(status) {
  return normalizeLiveRoomStatus(status) === LIVE_ROOM_STATUS.LIVE;
}
...
export async function getLiveRooms(options = {}) {
  ...
  if (status) {
    ...
    params.push(normalizeLiveRoomStatus(status));
    idx++;
  }
...
export async function createLiveRoom(data) {
  ...
  // 检查是否有正在直播的房间（live_rooms.status: pending=待开播 live=直播中 ended=已结束；兼容历史 1/2/3）
  const activeRoom = await repo.rawQuery(
    `SELECT * FROM live_rooms WHERE user_id = $1 AND status::text IN ('2', 'live') AND deleted_at IS NULL`,
    [data.userId]
  );
...
export async function startLive(id) {
  const room = await repo.findOne('live_rooms', { id, deletedAt: null });
  if (!room) throw new NotFoundError('直播间不存在');
  if (isLiveRoomActive(room.status)) throw new ConflictError('已在直播中');

  const updated = await repo.update('live_rooms', id, {
    status: LIVE_ROOM_STATUS.LIVE,
...
export async function endLive(id) {
  const room = await repo.findOne('live_rooms', { id, deletedAt: null });
  if (!room) throw new NotFoundError('直播间不存在');
  if (!isLiveRoomActive(room.status)) throw new ValidationError('未在直播中');

  const updated = await repo.update('live_rooms', id, {
    status: LIVE_ROOM_STATUS.ENDED,
...
export async function sendLiveMessage(roomId, data) {
  ...
  if (data.type === 1) {
    ...
    await repo.increment('live_rooms', roomId, 'viewCount', 1);
  }
