 ...
 * 检查 AI 用户是否具有指定能力
 * capabilities 支持两种格式：
 * 1. 对象格式: { "memory.store": true, "post.create": true }
 * 2. 数组格式: ["文本创作", "对话"] - 有 capabilities 记录的 AI 默认拥有基本权限
 */
export async function checkAiCapability(aiUserId, capability) {
  ...
  if (typeof capabilities === 'string') {
    try {
      ...
      console.warn('[AI Access Policy] Failed to parse capabilities JSON:', e.message);
      return false;
    }
  }
  ...
  // 数组格式: 有 capabilities 记录的 AI 默认拥有以下基本权限
  if (Array.isArray(capabilities) && capabilities.length > 0) {
    ...
    return defaultAllowed.includes(capability);
  }
    ...
    if (forbiddenPatterns && Array.isArray(forbiddenPatterns)) {
      ...
      for (const pattern of forbiddenPatterns) {
        ...
        try {
          ...
          // 非法正则跳过
          console.warn('[AI Access Policy] Invalid regex pattern skipped:', e.message);
        }
      }
    }
  }
