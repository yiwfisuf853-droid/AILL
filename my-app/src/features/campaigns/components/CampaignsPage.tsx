...
import { IconCampaign, IconClock, IconFire, IconStar, IconTrophy } from "@/components/ui/Icon";
import { PageSkeleton } from '@/components/ui/Skeleton';

export function CampaignsPage() {
  const user = useAuthStore(s => s.user);
  const { campaigns, achievements, loading, fetchCampaigns } = usePortalStore();

  // 兼容：campaigns store 用 activeTab，这里用本地 state
  ...
  async function handleJoin(_id: string) {
    toast('功能开发中');
  }
  ...
  return (
    <div className="py-3" data-name="campaigns">
      ...
      <div className="relative overflow-hidden border-b border-border" data-name="campaignsHero">
        <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, hsl(var(--warning) / 0.1) 0%, transparent 40%, hsl(var(--primary) / 0.05) 100%)' }} />
        <div className="absolute top-0 right-0 w-64 h-64 rounded-full blur-[100px] opacity-20" style={{ background: 'hsl(var(--warning))' }} />
        <div className="relative pt-10 pb-6" data-name="campaignsHeroContent">
          <div className="flex items-center gap-3 mb-1.5">
            <div className="flex items-center justify-center w-10 h-10 rounded-lg border" style={{ background: 'hsl(var(--warning) / 0.15)', borderColor: 'hsl(var(--warning) / 0.25)' }}>
              <IconCampaign size={20} style={{ color: 'hsl(var(--warning))' }} />
            </div>
            <h1 className="text-2xl font-bold tracking-tight" data-name="campaignsTitle">
              AILL <span className="textGradientBrand">Quests</span>
            </h1>
          </div>
                      ...
                      <div className="flex-1 min-w-0" data-name={`campaignsCampaign${c.id}Info`}>
                        ...
                        <div className="mb-3" data-name={`campaignsCampaign${c.id}Progress`}>
                          <div className="flex items-center justify-between text-xs text-foreground-tertiary mb-1.5">
                            <span data-name={`campaignsCampaign${c.id}ProgressCount`}>
                              {c.rewardConfig?.target ? `0 / ${c.rewardConfig.target}` : '—'}
                            </span>
                        ...
                        {rewards.length > 0 && (
                          <div className="flex items-center gap-2 flex-wrap" data-name={`campaignsCampaign${c.id}Rewards`}>
                            ...
                            {rewards.map((r: { amount: number; assetTypeId: number }, i: number) => (
                              <span key={i} className="tagPill" data-name={`campaignsCampaign${c.id}Reward${i}`} style={{ background: 'hsl(var(--warning) / 0.12)', color: 'hsl(var(--warning))' }}>
                        ...
                        {isActive && (
                          <button
                            ...
                            className={`mt-3 px-4 py-1.5 rounded-md text-sm font-medium transition-all bg-accent text-accent-foreground hover:opacity-90`}
                          >
                            参与任务
                          </button>
                        )}
                  ...
                  {a.reward?.rewards && a.reward.rewards.length > 0 && (
                    <div className="flex items-center gap-1 justify-center flex-wrap" data-name={`campaignsAchievement${a.id}Rewards`}>
                      {a.reward.rewards!.map((r: { amount: number; assetTypeId: number }, i: number) => (
                        <span key={i} className="tagPill" data-name={`campaignsAchievement${a.id}Reward${i}`} style={{ background: 'hsl(var(--warning) / 0.12)', color: 'hsl(var(--warning))' }}>
                          +{r.amount}
                        </span>
                      ))}
                    </div>
