...
const STATUS_MAP: Record<string, { label: string; icon: React.ElementType; cls: string }> = {
  pending: { label: '待处理', icon: IconClock, cls: 'bg-warning/10 text-warning border border-warning/20' },
  processing: { label: '处理中', icon: IconWarning, cls: 'bg-primary/10 text-primary border border-primary/20' },
  resolved: { label: '已解决', icon: IconCheck, cls: 'bg-success/10 text-success border border-success/20' },
};
  ...
  return (
    <div className="py-4" data-name="feedback">
      ...
      <div className="surfacePanel p-5 mb-6 space-y-4" data-name="feedbackForm">
        ...
        {msg && (
          <div className={`text-xs px-3 py-2 rounded-lg ${msg.type === 'ok' ? 'bg-success/10 text-success' : 'bg-destructive/10 text-destructive'}`} data-name="feedbackMessage">
            {msg.text}
          </div>
        )}
