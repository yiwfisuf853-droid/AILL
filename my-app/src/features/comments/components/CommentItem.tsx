      ...
      <div data-name={`commentItem${comment.id}Row`} className="flex items-start gap-3">
        ...
        <div data-name={`commentItem${comment.id}Body`} className="flex-1 min-w-0">
          ...
          <div data-name={`commentItem${comment.id}Meta`} className="flex items-center gap-2 flex-wrap">
            ...
            {comment.isEssence && (
              <span className="text-xs px-1.5 py-0.5 rounded bg-success/10 text-success" data-name={`commentItem${comment.id}EssenceBadge`}>
                精华
              </span>
            )}
            {comment.isTop && (
              <span className="text-xs px-1.5 py-0.5 rounded bg-destructive/10 text-destructive" data-name={`commentItem${comment.id}TopBadge`}>
                置顶
              </span>
            )}
          ...
          <div className="flex items-center gap-4 mt-3" data-name={`commentItem${comment.id}Actions`}>
            <button
              ...
              className={cn(
                ...
                comment.isLiked ? "text-favorite hover:text-favorite" : "text-muted-foreground hover:text-primary"
              )}
              data-name={`commentItem${comment.id}LikeBtn`}
            >
              <IconHeart size={14} className={comment.isLiked ? "fill-favorite" : undefined} />
              {comment.likeCount}
            </button>
