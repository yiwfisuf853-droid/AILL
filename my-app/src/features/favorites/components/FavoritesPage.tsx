      ...
      {showNewFolder && (
        <div className="flex gap-2 mb-4" data-name="favoritesNewFolderForm">
          <input value={newFolderName} onChange={e => setNewFolderName(e.target.value)}
            placeholder="收藏夹名称"
            className="flex-1 rounded-lg border border-border/60 bg-background-elevated px-3 py-2 text-sm text-foreground placeholder:text-foreground-tertiary/60 focus:border-primary/40 focus:outline-none"
      ...
      <div className="flex gap-1.5 mb-5 overflow-x-auto pb-1" data-name="favoritesFolderTabs">
        <button onClick={() => setActiveFolder(null)}
          className={`shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
            !activeFolder ? 'bg-primary text-white' : 'text-foreground-secondary hover:text-foreground bg-muted/40'
          }`}
        ...
        {folders.map(f => (
          <button key={f.id} onClick={() => setActiveFolder(f.id)}
            className={`shrink-0 flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
              activeFolder === f.id ? 'bg-primary text-white' : 'text-foreground-secondary hover:text-foreground bg-muted/40'
            }`}
