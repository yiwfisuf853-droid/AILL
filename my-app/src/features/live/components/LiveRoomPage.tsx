          ...
          <div data-name="liveRoomMainColumn" className="flex-1 min-w-0 flex flex-col gap-4">
            ...
            <div className="relative aspect-video bg-gradient-to-br from-muted to-background rounded-xl overflow-hidden border border-border/60" data-name="liveRoomVideo">
              ...
              <div data-name="liveRoomVideoCenter" className="absolute inset-0 flex items-center justify-center">
                {isLive && (
                  <div data-name="liveRoomLiveIndicatorWrap" className="flex flex-col items-center gap-3">
                    <div className="w-20 h-20 rounded-full bg-destructive/20 flex items-center justify-center backdrop-blur-sm border border-destructive/30 animate-pulse" data-name="liveRoomLiveIndicator">
                      <IconFire size={40} className="text-destructive" />
                    </div>
                ...
                {isScheduled && (
                  <div className="flex flex-col items-center gap-3" data-name="liveRoomScheduledIndicator">
                    <IconClock size={64} className="text-info/50" />
                    <span className="text-foreground-tertiary text-sm">直播预告</span>
                    {countdown && (
                      <span className="text-2xl font-mono text-info tracking-widest" data-name="liveRoomCountdown">{countdown}</span>
                    )}
                  </div>
                )}
              </div>
              ...
              {isLive && (
                <div data-name="liveRoomLiveBadgeWrap" className="absolute top-4 left-4">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-destructive/90 text-white shadow-lg shadow-destructive/30" data-name="liveRoomLiveBadge">
              ...
              {isScheduled && (
                <div data-name="liveRoomScheduledBadgeWrap" className="absolute top-4 left-4">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-info/20 text-info border border-info/30" data-name="liveRoomScheduledBadge">
              ...
              {isLive && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-48 h-24 bg-destructive/20 rounded-full blur-[60px] pointer-events-none" />
              )}
            </div>
            ...
            <div className="flex items-center justify-between gap-4 bg-card rounded-xl px-4 py-3 border border-border/60" data-name="liveRoomStreamerInfo">
              <div data-name="liveRoomStreamerMeta" className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-destructive to-primary flex-shrink-0 flex items-center justify-center text-sm font-bold overflow-hidden" data-name="liveRoomStreamerAvatar">
              ...
              <div data-name="liveRoomStreamerActions" className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                      followed
                        ...
                        : 'bg-gradient-to-r from-destructive to-primary text-white shadow-lg shadow-destructive/20'
                    }`}
                ...
                <div className="flex items-center gap-1 text-xs text-foreground-tertiary" data-name="liveRoomLikeCount">
                  <IconHeart size={14} className="text-destructive" /> {room.likeCount}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col bg-card rounded-xl border border-border/60 overflow-hidden" style={{ height: 400 }} data-name="liveRoomChat">
              ...
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-border/60 bg-card" data-name="liveRoomChatHeader">
                <div data-name="liveRoomChatHeaderTitle" className="flex items-center gap-2 text-sm font-medium text-foreground">
                  <IconComment size={16} className="text-destructive" />
                  弹幕聊天
                </div>
                  ...
                  messages.map((msg) => (
                    <div key={msg.id} data-name={`liveRoomChatMsg${msg.id}`} className="flex items-start gap-2 group">
                      {msg.type === 'system' ? (
                        ...
                        <div data-name={`liveRoomChatMsg${msg.id}Gift`} className="flex items-center gap-2 py-1 px-3 rounded-lg bg-primary/10 border border-primary/10 w-full">
                          <span className="text-lg">{msg.giftIcon || '🎁'}</span>
                          <span className="text-xs text-primary font-medium">{msg.username}</span>
                          <span className="text-xs text-foreground-tertiary">送出</span>
                          <span className="text-xs text-primary-hover font-semibold">{msg.giftName || msg.content}</span>
                        </div>
                      ) : (
                        <>
                          ...
                          <div data-name={`liveRoomChatMsg${msg.id}Text`} className="min-w-0">
                            <span className="text-xs text-destructive font-medium mr-1.5">{msg.username}</span>
              ...
              {isAuthenticated ? (
                <div className="flex items-center gap-2 px-3 py-2.5 border-t border-border/60 bg-card" data-name="liveRoomChatInput">
                  <input
                    ...
                    placeholder="发送弹幕..."
                    className="flex-1 bg-background border border-border/60 rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-foreground-tertiary/60 focus:outline-none focus:border-destructive/50 focus:ring-1 focus:ring-destructive/20 transition-all"
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-lg bg-gradient-to-r from-destructive to-primary text-white disabled:opacity-40 disabled:cursor-not-allowed hover:shadow-lg hover:shadow-destructive/20 transition-all"
                ...
                <div data-name="liveRoomChatLoginPrompt" className="px-4 py-3 border-t border-border/60 bg-card text-center">
                  <Link to="/login" className="text-destructive hover:text-destructive/80 text-sm" data-name="liveRoomLoginPrompt">
                    登录后即可发送弹幕
                  </Link>
                </div>
              )}
            ...
            <div className="bg-card rounded-xl border border-border/60 overflow-hidden flex flex-col" style={{ height: '100%' }} data-name="liveRoomGiftPanel">
              ...
              {!isAuthenticated && (
                <div data-name="liveRoomGiftLoginPromptWrap" className="px-4 py-3 border-t border-border/60 bg-card text-center">
                  <Link to="/login" className="text-destructive hover:text-destructive/80 text-xs" data-name="liveRoomGiftLoginPrompt">
                    登录后即可送礼物
                  </Link>
                </div>
              )}
            ...
            <div className="mt-2 bg-card rounded-xl border border-border/60 p-3" data-name="liveRoomMobileGiftPanel">
              ...
              {!isAuthenticated && (
                <div data-name="liveRoomMobileGiftLoginPromptWrap" className="text-center mt-3">
                  <Link to="/login" className="text-destructive hover:text-destructive/80 text-xs" data-name="liveRoomMobileGiftLoginPrompt">
                    登录后即可送礼物
                  </Link>
                </div>
              )}
