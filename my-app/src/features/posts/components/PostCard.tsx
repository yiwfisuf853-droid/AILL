...
export const PostCard = memo(function PostCard({ post, variant = 'default' }: PostCardProps) {
  ...
  const detailHref = post.id ? `/posts/${post.id}` : '/square';
  return (
    <article data-name={`postCard${post.id || 'unknown'}`} className="border border-border rounded-xl overflow-hidden hover:border-border-hover transition-all duration-200">
      <Link to={detailHref} data-name={`postCard${post.id || 'unknown'}Link`} className="block p-3">
        {/* ── Top: Author + Badges ── */}
        <div data-name={`postCard${post.id}Header`} className="flex items-center gap-2 mb-2">
          ...
          <div data-name={`postCard${post.id}Meta`} className="flex items-center gap-1 min-w-0 flex-1">
            ...
            {section && (
              <span
                ...
                className="inlineChip text-[8px] shrink-0 bg-primary/10 text-primary"
              >
                {section.name}
              </span>
            )}
          ...
          <div data-name={`postCard${post.id}Badges`} className="ml-auto flex items-center gap-0.5 shrink-0">
            ...
            {post.isHot && (
              <span data-name={`postCard${post.id}HotBadge`} className="inlineChip text-[8px] bg-warning/10 text-warning border border-warning/20">
                <IconFire size={8} /> 热
              </span>
            )}
            {post.isEssence && (
              <span data-name={`postCard${post.id}EssenceBadge`} className="inlineChip text-[8px] bg-success/10 text-success border border-success/20">
                <IconStar size={8} /> 精
              </span>
            )}
          </div>
            ...
            {!isCompact && post.tags?.length > 0 && (
              <div data-name={`postCard${post.id}Tags`} className="flex flex-wrap gap-1 mt-1.5">
                {post.tags.slice(0, 3).map((tag) => (
                  <span
                    key={tag}
                    data-name={`postCard${post.id}Tag${tag}`}
                    className="tagPill text-[9px] hover:bg-accent transition-colors cursor-pointer"
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      window.location.href = `/square?tag=${encodeURIComponent(tag)}`;
                    }}
                  >#{tag}</span>
                ))}
              </div>
        ...
        <div data-name={`postCard${post.id}Actions`} className="flex items-center gap-0.5 mt-2 pt-1.5 border-t border-border/30 -ml-0.5">
          <span data-name={`postCard${post.id}LikeBtn`} className="actionBtn !gap-0.5 !text-xs">
            <IconHeart size={11} className={post.isLiked ? 'fill-favorite text-favorite' : ''} /> {post.likeCount}
          </span>
