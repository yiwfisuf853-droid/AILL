...
const SORT_OPTIONS = [
  { value: 'hot', label: '热门', icon: IconFire, color: 'var(--primary)' },
  { value: 'latest', label: '最新', icon: IconClock, color: 'var(--info)' },
  { value: 'essence', label: '精华', icon: IconStar, color: 'var(--success)' },
];
...
const SECTION_TABS = [
  { id: '', name: '全部', icon: '📰', color: 'var(--primary)' },
  ...SECTIONS.map(s => ({ id: s.id, name: s.name, icon: s.icon, color: 'var(--primary)' as string })),
];
...
export function PostListPage() {
  ...
  // 从 URL 读取 tag/search 参数
  useEffect(() => {
    const tag = searchParams.get('tag');
    const kw = searchParams.get('keyword');
    if (tag) setSearchQuery(tag);
    else if (kw) setSearchQuery(kw);
  }, [searchParams]);
  ...
  const activeSortColor = SORT_OPTIONS.find(o => o.value === sortBy)?.color || 'var(--primary)';
    ...
    <div data-name="postList" className="py-3">
      ...
      <div data-name="postListHeader" className="flex items-center gap-2 mb-3">
        <div
          ...
          style={{ background: `hsl(${activeSection?.color || 'var(--primary)'})` }}
        />
          ...
          return (
            <button
              ...
              style={isActive ? {
                ...
                boxShadow: `0 1px 6px hsl(${option.color} / 0.25)`,
              } : undefined}
            >
              <Icon size={12} />
              {option.label}
      ...
      <div data-name="postListPostList" className="space-y-1.5">
        {loading && posts.length === 0 ? (
          ...
          <EmptyState type="search" title="暂无帖子" description="试试其他筛选条件" />
        ) : (
          <>
            {posts.map((post: Post) => (
