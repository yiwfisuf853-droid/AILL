...
import type { Comment, CommentCreateDto, CommentListQuery } from '@/features/comments/types';
...
export const usePostsStore = create<PostsState>((set, get) => ({
  ...
  likeComment: async (id: string) => {
    if (get().likeLoading[id]) return;
    set((state) => ({ likeLoading: { ...state.likeLoading, [id]: true } }));
    try {
      await commentApi.likeComment(id);
      set((state) => ({
        comments: state.comments.map((c) =>
          c.id === id ? { ...c, isLiked: !c.isLiked, likeCount: c.isLiked ? c.likeCount - 1 : c.likeCount + 1 } : c
        ),
        likeLoading: { ...state.likeLoading, [id]: false },
      }));
    } catch {
      set((state) => ({ likeLoading: { ...state.likeLoading, [id]: false } }));
    }
  },
  ...
  clearComments: () => set({ comments: [], commentsTotal: 0 }),
}));
