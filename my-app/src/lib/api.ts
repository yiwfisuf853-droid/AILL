...
// 请求拦截器 - 注入 Token（不再自动转换键名，后端 validateRequest 中间件统一处理）
...
// 响应拦截器 - 转换响应体 + 统一错误处理
// 注意：请求端不再做 camelCase → snake_case 转换，键名保持 camelCase 直传后端
// 后端 validateRequest 中间件统一做 snake_case → camelCase 反向转换
