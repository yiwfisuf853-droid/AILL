...
import { NAV_LINKS, MODULE_HSL, SECTIONS, getSidebarConfig, type SidebarConfig } from "@/lib/navConfig";
import {
...
export function Sidebar() {
  ...
  useEffect(() => {
    const handler = () => setSidebarConfig(getSidebarConfig());
    window.addEventListener('sidebarConfigChanged', handler);
    return () => window.removeEventListener('sidebarConfigChanged', handler);
  }, []);
  ...
  const content = (
    <div data-name="sidebar" className="flex flex-col h-full">
      ...
      <div data-name="sidebarLogo" className="flex items-center gap-2.5 px-3 h-14 shrink-0 border-b border-border/40 bg-card/50 backdrop-blur-sm">
        ...
        >
          <span data-name="sidebarLogoLetter" className="text-sm font-black transition-colors duration-300" style={{ color: `hsl(${activeHsl})` }}>
            A
          </span>
        </div>
        {!collapsed && (
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 space-y-1.5">
        ...
        <nav data-name="sidebarMainNav" className="space-y-0.5">
          {NAV_LINKS.filter(link => link.module !== 'sections').map((link) => {
            ...
            return (
              <Link
                ...
                data-name={`sidebarNav${link.module}${active ? 'Active' : ''}`}
                className="sidebarNavItem relative"
        ...
        {/* 发帖按钮 */}
        {isAuthenticated && (
          <div data-name="sidebarCreatePost" className={collapsed ? "px-0.5" : "px-1"}>
            <Link
              to="/posts/create"
              onClick={() => mobileOpen && setMobileOpen(false)}
              data-name="sidebarCreatePostBtn"
        ...
        <div data-name="sidebarPersonal" className="pt-2 border-t border-border/40 space-y-0.5">
          ...
          {isAuthenticated ? (
            <>
              {visiblePersonal.map((item) => {
                switch (item.id) {
                  case 'notifications':
                    return (
                      <Link key={item.id} to="/notifications" onClick={() => mobileOpen && setMobileOpen(false)} data-name="sidebarNotifications" className="sidebarNavItem" title={collapsed ? "通知" : undefined}>
                        <div className="relative">
                          <IconNotification size={18} />
                          {unreadCount > 0 && (
                            <span data-name="sidebarNotificationsBadge" className="absolute -top-1 -right-1 min-w-[14px] h-3.5 flex items-center justify-center rounded-full bg-red-500 text-white text-[9px] font-bold px-0.5">
                  ...
                  case 'messages':
                    return (
                      <Link key={item.id} to="/messages" onClick={() => mobileOpen && setMobileOpen(false)} data-name="sidebarMessages" className="sidebarNavItem" title={collapsed ? "私信" : undefined}>
                        <div className="relative">
                          <IconMail size={18} />
                          {unreadMessages > 0 && (
                            <span data-name="sidebarMessagesBadge" className="absolute -top-1 -right-1 min-w-[14px] h-3.5 flex items-center justify-center rounded-full bg-red-500 text-white text-[9px] font-bold px-0.5">
                  ...
                  case 'favorites':
                    return (
                      <Link key={item.id} to="/favorites" onClick={() => mobileOpen && setMobileOpen(false)} data-name="sidebarFavorites" className="sidebarNavItem" title={collapsed ? "收藏" : undefined}>
                        <IconHeart size={18} />
                  ...
                  case 'profile':
                    return user ? (
                      <Link key={item.id} to={`/users/${user.id}`} onClick={() => mobileOpen && setMobileOpen(false)} data-name="sidebarProfile" className="sidebarNavItem" title={collapsed ? "我的" : undefined}>
                        <IconUser size={18} />
                  ...
                  case 'settings':
                    return (
                      <Link key={item.id} to="/settings" onClick={() => mobileOpen && setMobileOpen(false)} data-name="sidebarSettings" className="sidebarNavItem" title={collapsed ? "设置" : undefined}>
            ...
            !collapsed && (
              <div data-name="sidebarAuth" className="px-1 space-y-1.5">
                <Link to="/auth/login" onClick={() => mobileOpen && setMobileOpen(false)} data-name="sidebarLoginBtn" className="sidebarNavItem w-full justify-center border border-border">
                  登录
                </Link>
                <Link
                  to="/auth/register"
          ...
          {/* 主题切换 + 退出 */}
          <button onClick={toggleTheme} data-name="sidebarThemeToggle" className="sidebarNavItem w-full" title={collapsed ? (theme === "dark" ? "亮色" : "暗色") : undefined}>
