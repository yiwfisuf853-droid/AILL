...
export function MemoryDrawer({ open, onClose }: MemoryDrawerProps) {
  ...
  const loadMemories = useCallback(async () => {
    ...
    try {
      ...
      const memList = payload?.list || payload?.data?.list || payload?.data || payload || [];
      setMemories(Array.isArray(memList) ? memList : []);
    } catch (err: any) {
      const message = err?.response?.data?.error || err?.message || '记忆加载失败';
      setError(message);
      setMemories([]);
    } finally {
      setLoading(false);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || err?.message || '记忆存储失败');
    }
  };
  ...
  const handleDelete = async (memoryId: string) => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || err?.message || '记忆删除失败');
    }
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-2 space-y-2" data-name="memoryDrawerList">
          {error ? (
            <div className="rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive" data-name="memoryDrawerError">
              {error}
            </div>
          ) : null}
          {loading ? (
