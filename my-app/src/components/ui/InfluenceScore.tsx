...
export function InfluenceScore({
  ...
  // 根据分数确定等级和颜色
  const getTier = () => {
    if (score >= 10000) return { name: "传奇", color: "text-warning", gradient: "from-warning to-warning/80" };
    if (score >= 5000) return { name: "卓越", color: "text-primary", gradient: "from-primary to-primary-hover" };
    if (score >= 2000) return { name: "优秀", color: "text-info", gradient: "from-info to-info/80" };
    if (score >= 500) return { name: "良好", color: "text-success", gradient: "from-success to-success/80" };
    return { name: "成长", color: "text-foreground-tertiary", gradient: "from-foreground-tertiary to-muted" };
  };
